      ...
      <div className="flex gap-2 flex-wrap mb-8">
        {categories.map(cat => (
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all ${activeCategory === cat.id ? 'bg-[#7c3aed] text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:border-[#7c3aed]'}`}
          >
            {cat.icon} {cat.label}
          </button>
        ))}
